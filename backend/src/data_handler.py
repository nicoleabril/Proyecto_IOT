...
class DataHandler(metaclass=SingletonMeta):
    def __init__(self):
        self.temp_subscribers = []  # Cambio: corregido el nombre de la lista de suscriptores de temperatura
        self.hum_subscribers = []

    def send_temp(self, temp):
        for sub in self.temp_subscribers:
            try:
                sub.send_json({'data': f'{temp:.2f}'})  # Cambio: enviar el dato de temperatura correctamente formateado
